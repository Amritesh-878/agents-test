...
## Status — 2026-07-03 (A/B model comparison for safe swaps)

Added a per-case old-vs-new signal for forced Groq model swaps, on top of the unified retrieval. Stage: pre-release alpha, local-only, real student PII. **377 tests, ruff/mypy clean.** All work on `main`.

### Done — `scripts/compare_models.py`
- A model swap changes only GENERATION, so the tool holds retrieval fixed: each case is retrieved **once** (via the committed `RetrievalBackend` path, so `effective_top_k` widening applies exactly as in the CLI/demo), then the same context is generated by the `--baseline-model` and `--candidate-model` and scored with the **same** answer-expectation checks the eval harness uses (reuses `evaluate_answer_expectations` / `evaluate_retrieval_expectations` / `build_failure_reasons` / `load_indexed_chunks`, extracted to a shared module function).
- Classifies every case **REGRESSION** (passed baseline, failed candidate) / **IMPROVEMENT** (failed, then passed) / **UNCHANGED**, emits `comparison.json` + `comparison.md` (summary counts on top, per-case both answers + matched/missing concepts + forbidden hits), and **exits nonzero on any REGRESSION** so it can gate a swap. Zero-retrieval cases share the deterministic no-evidence fallback (no model call), so a retrieval miss never reads as a model regression.

### Done — golden-set expansion (sensitivity)
- The old golden set was all self-referential / refusal (top_k stays at base 5), so it never exercised the general-question path the k=8 widening exists for. Added three GENERAL cases grounded in **live-retrieved class_context** (not invented): `saisha-general-what-covered`, `saisha-general-teacher-instruction` (Math worksheet / linear-equation / teacher instructions) and `bhagyashree-general-determinants` (Economics determinants: taxes, technology, price of related goods). All are non-self-referential, so they now retrieve at k=8 live and cover the demo's real phrasings ("what did we cover", worksheet/topic, "what did the teacher ask us to do").

### Tests
- Classifier over all four transitions (no live model); markdown reports summary + classifications.
- `compare_case` retrieves **once** and generates **twice** (mock store/embedder + fake backend: one search call, two generate calls with the two model ids); zero-retrieval shares the fallback with no model call.
- The three new golden cases load + validate under `EvalDataset`, are non-self-referential, grounded (concept groups present), and unfiltered.

### Verified (live, DB + Groq)
- Baseline unregressed: `python -m scripts.evaluate --output-dir output/eval_ab_base` → **8/8** (the 3 new general cases pass; they retrieve at k=8).
- Tool proven by A/B-ing current vs the deprecated predecessor: `python -m scripts.compare_models --baseline-model llama-3.1-8b-instant --candidate-model openai/gpt-oss-20b` → **REGRESSION 0, IMPROVEMENT 0, UNCHANGED 8** (both models pass all 8), exit 0.

### Runbook — next forced Groq swap
- Run `./.venv/Scripts/python.exe -m scripts.compare_models --baseline-model <current> --candidate-model <new>` (needs `GROQ_API_KEY` + local Postgres). Read `comparison.md`; **block the swap on any REGRESSION** (the process also exits nonzero). Then flip `DEFAULT_GROQ_MODEL` in `scripts/chat.py` and re-run `scripts.evaluate` to confirm the new default is unregressed.
