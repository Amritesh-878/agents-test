"""A/B model comparison for safe Groq model swaps.

A model swap changes only GENERATION, so retrieval is held fixed: each case is
retrieved ONCE (via the committed CLI retrieval path, ``effective_top_k`` widening and
all), then the same retrieved context is generated by the baseline and candidate models
and scored with the SAME answer-expectation checks the eval harness uses. Every case is
classified REGRESSION / IMPROVEMENT / UNCHANGED so a forced swap has a per-case
old-vs-new signal, not just a binary pass/fail. Exits nonzero on any REGRESSION so it can
gate a swap.
"""
...
def classify_case(passed_baseline: bool, passed_candidate: bool) -> Classification:
    """Classify one case's old-vs-new outcome.

    A case counts as a REGRESSION only when the baseline passed and the candidate then
    failed (the swap made it worse); IMPROVEMENT is the reverse; everything else (both
    pass, or both fail — e.g. a shared retrieval miss that is not model-dependent) is
    UNCHANGED.
    """
...
class ComparisonService:
    def __init__(
        ...
        *,
        ...
        # Only load the API key when we must build a real backend, so unit tests that
        # inject a fake backend need no GROQ_API_KEY.
    ...
    def compare_case(self, case: EvalCase) -> CaseComparison:
        ...
        # Retrieval is model-independent: run it ONCE and score both models against it.
        ...
        if retrieval_result.result_count == 0:
            # No context: the grounded "no evidence" fallback is deterministic and
            # model-independent, so both models get the identical answer (no Groq call).
